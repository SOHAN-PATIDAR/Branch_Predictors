...
inline void O3_CPU::initialize_branch_predictor()
{
    tage_predictor[cpu].init();
}

inline uint8_t O3_CPU::predict_branch(uint64_t ip)
{
    return tage_predictor[cpu].predict(ip);
}

inline void O3_CPU::last_branch_result(uint64_t ip, uint64_t branch_target, uint8_t taken, uint8_t branch_type)
{
